...
const urlsToSkip = ["https://www.useblackbox.io", "/login", "/signup", "/signin"]

const regex = new RegExp(urlsToSkip.join('|'))
chrome.storage.sync.get(function (items) {
	var domainsHidden = items.domainsHidden
	if (!domainsHidden) {
		domainsHidden = []
	}
	if (!items.hideChat && !domainsHidden.includes(window.location.origin) && !regex.test(url)) {
		chrome.runtime.sendMessage({message:"initChat"})
		createBlackboxButton()
	}
})
...
function createBlackboxButton() { 
	const $overlay = $(`
		<section class="blackbox-overlay">
			<div class="drawer-control-x-button blackbox-button-holder" style="display: flex;">
				<img src="${chrome.extension.getURL("images/logo-vertical.svg")}" loading="lazy" alt="" class="image-36">
			</div>
			<div data-current="Tab 1" data-easing="ease" data-duration-in="300" data-duration-out="100" class="tabs-2 w-tabs">
				<div class="chrome-heading-menu">
					<img src="${chrome.extension.getURL("images/logo_1.svg")}" loading="lazy" alt="" class="image-34">
					<div class="right-holder">
						<div style="font-size:10px; cursor:pointer;color:rgba(255, 255, 255, 0.8);" class="open-confirm-modal">Settings</div>
						<div class="link-block-7 close-overlay">
							<img src="${chrome.extension.getURL("images/chevron-right.svg")}" loading="lazy" alt="">
						</div>
					</div>
				</div>
				<div class="confirmation-modal">
					<div class="text-block-15">Turn off Blackbox?</div>
					<div class="confirm-button session-close">Yes for this session</div>
					<div class="confirm-button domain-close">Yes for this domain</div>
					<div class="confirm-button perm-close">Yes for all domain</div>
					<div class="confirm-button plain-button modal-close">No, don't turn it off</div>
					<div class="small-close on-confirmation-modal w-inline-block">
						<img src="https://uploads-ssl.webflow.com/647b75407a16be15a82901be/6489f7e3cd2fb16b87351cac_Close.svg" loading="lazy" alt="" class="image-37">
					</div>
				</div>
				<div class="tabs-content-2">
					...
					<div class="div-block-69">
						<div class="chatlog-container chat-area">
							<div class="vscode-chartbox">
								<div class="div-block-68">
								<div class="text-block-10">Hi! Here some our recommendations to get the best out of BLACKBOX:</div>
								</div>
								<div class="indent">
								<div class="div-block-68"><img alt="" loading="lazy" src="${chrome.extension.getURL("images/check_1.svg")}" class="icon-vscode">
									<div class="text-block-10">Be as clear as possible</div>
								</div>
								<div class="div-block-68"><img alt="" loading="lazy" src="${chrome.extension.getURL("images/check_1.svg")}" class="icon-vscode">
									<div class="text-block-10">End the question in what language you want the answer to be, e.g: ‘connect to mongodb in python</div>
								</div>
								</div>
								<div class="chat-button-containber">
								<div class="text-block-7 no-margin">or you can just</div>
								<a href="https://www.youtube.com/watch?v=VDj4fspm808" target="_blank" class="vscode-button white">Watch tutorial video</a>
								</div>
							</div>
							...
							<div class="vscode-chartbox right-no-outline starter-text">
								<div class="text-block-10">How to delete docs from mongodb in phyton</div>
							</div>
							<div class="vscode-chartbox right-no-outline starter-text">
								<div class="text-block-10">Connect to mongodb in nodejs</div>
							</div>
						</div>
							...
							<div class="chatfield-container">
								<div class="form-block w-form">
									<div id="email-form" class="form">
										<textarea type="text" class="text-field-copy send-message" autofocus="true" placeholder="Ask any coding question" ></textarea>
										<span style="cursor: pointer" class="button-circle send-message-icon">
											<img loading="lazy" src="${chrome.extension.getURL("images/send.svg")}" alt="send" class="icon-24">
										</span>
										<span style="cursor: pointer" class="button-circle reset-chat">
											<img loading="lazy" src="${chrome.extension.getURL("images/refresh-ccw.svg")}" alt="refresh" class="icon-24 reset-chat">
										</span>
									</div>
								</div>
							</div>
							<div style="font-size:8px" class="text-block-10-copy">Blackbox AI Chat is in beta and Blackbox is not liable for the content generated. By using Blackbox, you acknowledge that you agree to agree to Blackbox&#x27;s <a tyle="color:inherit" href="https://www.useblackbox.io/terms" target="_blank" class="link-item">Terms</a> and <a style="color:inherit" href="https://www.useblackbox.io/privacy" class="link-item">Privacy Policy</a>
							</div>
						</div>
					</div>
					</div>
	...
	`)
		...
		$overlay.find(".send-message").on("keydown", function(e){
			const key = e.key
		...
		$overlay.find(".send-message-icon").on("keydown", function(e){
		...
		$overlay.find(".send-message").on('input', function(){
			$(this).height(1);
			var totalHeight = $(this).prop('scrollHeight') - parseInt($(this).css('padding-top')) - parseInt($(this).css('padding-bottom'));
			$(this).height(totalHeight);
		})
		$overlay.find(".starter-text").on("click", function(){
		...
		$overlay.find(".blackbox-button-holder").on("click", function (e) {
			if (!$(e.target).hasClass("confirmation-btn") && !$(e.target).hasClass("image-37") && !$overlay.find(".blackbox-button-holder").hasClass("active")) { 
				$overlay.addClass("active")
			}
		})
		$overlay.find(".reset-chat").on("click", function () { 
			$overlay.find(".chatMessage").remove()
			continueObj = {}
			lastAnswer = ""
			allAnswers = []
			lastSearchedQuery = ""
		...
		$overlay.find(".remove-once").on("click", function () { 
			removeOnce($overlay)
		})
		$overlay.find(".remove-perm").on("click", function () { 
			removePerm($overlay)
		})
		$overlay.find(".open-confirm-modal").on("click", function () { 
			showConfirmation($overlay)
		})
		$overlay.find(".on-confirmation-modal").on("click", function () { 
			hideConfirmation($overlay)
		})
		$overlay.find(".modal-close").on("click", function () { 
			hideConfirmation($overlay)
		})
		$overlay.find(".session-close").on("click", function () { 
			removeOnce($overlay)
			chrome.runtime.sendMessage({message:"perference", prefStatus: 'session'})
		})
		$overlay.find(".perm-close").on("click", function () { 
			removePerm($overlay)
			chrome.runtime.sendMessage({message:"perference", prefStatus: 'all'})
		})
		$overlay.find(".domain-close").on("click", function () { 
			removeDomain($overlay)
			chrome.runtime.sendMessage({message:"perference", prefStatus: 'domain'})
		})
	...
	setTimeout(() => {
		$("head").append(`
			...
			<link rel="stylesheet" href='${chrome.extension.getURL("css/dracula.css")}' />
			<link rel="stylesheet" href='${chrome.extension.getURL("css/codemirror.css")}' />
		`)

		var loadCmString = loadCm.toString();
		var loadJsString = loadJs.toString();

		$("body").append(`
			<script>
				var loadCm = eval(${loadCmString});
				var loadJs = eval(${loadJsString});
			
				
				var CodeMirror = loadCm();
...
function showConfirmation($holder) {
	$holder.find(".blackbox-button-holder").addClass("active")
	$holder.find(".confirmation-modal").addClass("active")
}
function hideConfirmation($holder) { 
	setTimeout(() => {
		$holder.find(".blackbox-button-holder").removeClass("active")
	}, 100);
	$holder.find(".confirmation-modal").removeClass("active")
}

function removeOnce($holder) { 
	$holder.removeClass("active")
	setTimeout(() => {
		$holder.remove()
	}, 300);
}
function removePerm($holder) {
	chrome.storage.sync.set({ hideChat: true })
	$holder.removeClass("active")
	setTimeout(() => {
		$holder.remove()
	}, 300);
}
function removeDomain($holder) {
	chrome.storage.sync.get(function (items) { 
		...
		if (!domainsHidden.includes(url)) { 
			domainsHidden.push(url)
		}

		chrome.storage.sync.set({ domainsHidden })
	})
	$holder.removeClass("active")
	setTimeout(() => {
		$holder.remove()
...
function addHolder(data){
	...
	$holder.find(".bottom-holder").html(`<div class='typing'>blackbox typing<img src='${chrome.extension.getURL("images/loader_three-dots-white.svg")}' class='typing-loading'></div><div class='date-display'>${dateDisplay}</div>`)
...
function getAnswer(data) {
	...
	continueObj = data
	try {
		chrome.runtime.sendMessage({message:"getAnswer", textInput: message, allMessages, stream, clickedContinue, max_iterations, iterations, completed, id})
	} catch (e) {
		console.log(e)
	}
}
...
chrome.runtime.onMessage.addListener((req, sender, res) => {
	if (req.message === "showAnswer") {
		const data = req.data
		const stream = JSON.parse(data.result).response[0][0]
		...
		formatMessage({ response: JSON.parse(data.result).response[0][0], completed: true, id: data.id, clickedContinue: false })
		if(data.iterations < data.max_iterations){
			if(takeInAnswers){
				lastStream = stream
				getAnswer({message, allMessages, max_iterations: data.max_iterations, iterations: data.iterations + 1, completed: true, stream, id: data.id})
			}
			else{
				// removeTyping({id})
			}
	...
	if(!$(".quality-toaster").length){
		...
		$el.find(".toaster-icon").on("click", function(){
			...
			$.ajax({
				...
				data:JSON.stringify({feedback: type, query: lastSearchedQuery, result: lastAnswer, userId: user._id}),
...
function removeTyping(data){
	const {id} = data
	document.querySelector('.chatMessage[data-id="' + id + '"]').querySelector('.typing').style.display = 'none';
}
...
function receiveStream(data) {
	...
	answerArr.forEach((answerObj, idx)=>{
		...
		if(type === "code"){
			...
			const $topBar = $(`<div class="top-code-bar"><div class="copy-text">Copied!</div><img class="code-copy-img" src="${chrome.extension.getURL("images/copy.svg")}"></div>`)
			...
			const $script = $(`
				<script>
					document.querySelectorAll(".pre-cm").forEach((el, idx)=>{
