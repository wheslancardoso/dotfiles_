...
import { useRef, useState, useImperativeHandle, useCallback, forwardRef } from "react";
...
const MODE_TO_TYPE: ReadonlyMap<Mode, CardType> = new Map([
	["paraphrase", "wide"],
	["correct", "narrow"],
]);
...
const Selector: React.FC<SelectorProps> = ({ mode, onModeChange }) => {
	const { hasCustomServer } = useStorageContext();
			...
			<Div className="lt-comp-card-base__selector">
				<Div
					className={classes(
						...
						hasCustomServer() && "lt-comp-card-base__selector__label--disabled"
					)}
					onClick={hasCustomServer() ? undefined : toggleMenu}
				>
...
const DragHandleBase: React.ForwardRefRenderFunction<HTMLElement, { isVisible: boolean }> = ({ isVisible }, ref) => {
	if (isVisible) {
		return (
...
const CardBase = forwardRef<CardBaseRef, React.PropsWithChildren<Props>>(function CardBase(
	...
	const draggableInstance = useDraggable(cardBaseRef.current, dragHandleRef.current);
	...
	const handleModeChange = isFunction(onModeChange)
		? (newMode: Mode) => {
				const newCardType = MODE_TO_TYPE.get(newMode);

				if (newCardType && newCardType !== cardType) {
	...
	useImperativeHandle<object, CardBaseRef>(
		...
		[draggableInstance.isDetached, updateCardPosition]
	);
		...
		>
			...
			>
				<LtCompCardBase
					...
					ref={cardBaseRef}
				>
					<DragHandle isVisible={draggable === true} ref={dragHandleRef} />
