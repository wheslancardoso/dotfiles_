...
const PREMIUM_URL = "https://languagetool.org/premium?pk_campaign=addon2-rewriting";

const getAvailableFiltersFromPhrases = (paraphrasings: PhraseResult | undefined): ParaphraseType[] => {
	return uniq(
		paraphrasings?.phrases
...
const Loader: React.FC = () => {
	...
	const sentence = (
		<Div className="lt-comp-paraphrasing-content__loading">
...
const Filter: React.FC<FilterProps> = ({ hidden, current, availableFilters, onChange }) => {
	...
	return (
		<>
			<Div className="lt-comp-paraphrasing-content__filter">
...
const ParaphrasingContent: React.FC<Props> = ({
	...
	const phrases = paraphrasings?.phrases.filter(({ label }) => !currentFilter || label.startsWith(currentFilter));
	...
	const title = useMemo<React.ReactNode>(
		() => (
			<CardContentTitle>
				...
				<Filter
					hidden={!paraphrasings || paraphrasings.phrases.length === 0}
		...
		[paraphrasings, currentFilter, setCurrentFilter, i18n.phrasesCardHeadline]
	);
	...
	if (remainingRephrasings === 0) {
		return (
			<CardContent className="lt-comp-paraphrasing-content">
