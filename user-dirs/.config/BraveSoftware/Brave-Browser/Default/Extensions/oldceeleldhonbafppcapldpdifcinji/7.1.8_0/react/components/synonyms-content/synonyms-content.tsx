...
const SynonymsContent: React.FC<Props> = ({
	...
	const shouldDisplayToggleMore = useMemo(
		() => synonyms && (synonyms.synonymSets.length > 1 || showFullSynonymSetInitially(synonyms.synonymSets[0])),
		[synonyms]
	);
				...
				.map((synonymSet, i, { length }, j = length - 1) => (
					...
					>
						...
						<Alternatives
							...
							showAll={showAll || showFullSynonymSetInitially(synonymSet) || !shouldDisplayToggleMore}
							synonymsSource={i === j && showAll ? synonymsSource : undefined}
