...
import { ChangeLanguageAction, ErrorFix, ErrorType } from "./types";
import type { RephraseObject, TextError } from "../../../core/Checker";
...
import type { UserForRewriteInterface } from "../../../background/phrases";
...
export interface Props {
	...
	shouldRequestSentenceSplittingSuggestion: boolean;
	isSentenceSplittingEnabled: boolean;
	inhouseOnly: boolean;
	uniqueId: string;
	showRuleId: boolean;
	language: string;
	user?: UserForRewriteInterface;
...
const ErrorFixButton: React.FC<{ fix: ErrorFix }> = ({ fix }) => {
	...
	const buttonProps = {
		className: classes(
			...
			fix.type === "optional" && "lt-error-content__suggestion--optional",
...
const ErrorContent: React.FC<Props> = ({
	...
	isHiddenMatch,
	premiumErrors,
	shouldRequestSentenceSplittingSuggestion,
	isSentenceSplittingEnabled,
	language,
	inhouseOnly,
	uniqueId,
	showRuleId,
	...
	const caption = useCaption(title, errorType, hasPremiumErrorsOnly, disablePremiumTeaser);
	const getMessage = useI18nContext();
	...
	const shouldShowSentenceSplittingContent =
		isSentenceSplittingEnabled && shouldRequestSentenceSplittingSuggestion && originalPhrase;
	...
	return (
		<Div>
			<CardContent caption={caption} controls={cardControls} footer={Footer}>
				...
				{!shouldRequestSentenceSplittingSuggestion && !hasError && (
					<EmptyState
			...
			{shouldShowSentenceSplittingContent && (
